...
<body>
  ...
  <a href="https://idouyin.io/34" target="_self">高速科学上网->狂舞机场</a>
</body>`
...
async function handleRequest(request) {
  ...
  if(!path){
    ...
    const html= await fetch("https://yomige.github.io/Url-Shorten-Worker/"+config.theme+"/index.html")
  ...
  if (location) {
    if (config.safe_browsing_api_key){
      if(!(await is_url_safe(location))){
        let warning_page = await fetch("https://yomige.github.io/Url-Shorten-Worker/safe-browsing.html")
    ...
    if (config.no_ref=="on"){
      let no_ref= await fetch("https://yomige.github.io/Url-Shorten-Worker/no-ref.html")
